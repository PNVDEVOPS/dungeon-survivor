...
function drawSprite(id, x, y, w, h, flipX) {
  ...
  if (flipX) {
    ...
    ctx.drawImage(img, rx - rw / 2, ry - rh / 2, rw, rh);
  }
  return true;
}
...
function drawSpriteAnim(id, x, y, w, h, animTick, flipX, row) {
  ...
  if (flipX) {
    ctx.save();
    ctx.translate(rx, ry); ctx.scale(-1, 1);
    ctx.drawImage(img, frame * sd.fw, useRow * sd.fh, sd.fw, sd.fh, -rw / 2, -rh / 2, rw, rh);
    ctx.restore();
    ctx.imageSmoothingEnabled = false;
  } else {
    ctx.drawImage(img, frame * sd.fw, useRow * sd.fh, sd.fw, sd.fh, rx - rw / 2, ry - rh / 2, rw, rh);
  }
  return true;
}
...
// variant = random int to pick a frame from spritesheet (for tree variants etc.)
function drawWorldObjSprite(x, y, objType, scale, variant) {
  let sprId = null;
  ...
  if (!sprId || !SPR[sprId]) return false;
  const s = scale || 1;
  const baseSz = objType === 'tree' ? 40 : objType === 'rock' ? 28 : 36;
  const sz = baseSz * s;
  // If spritesheet, pick a specific frame (variant)
  const sd = SHEET[sprId];
  if (sd && sd.total > 1) {
    const frame = (variant || 0) % sd.total;
    const col = frame % sd.cols;
    const row = Math.floor(frame / sd.cols);
    const img = SPR[sprId];
    const rsz = Math.round(sz);
    X.imageSmoothingEnabled = false;
    X.drawImage(img, col * sd.fw, row * sd.fh, sd.fw, sd.fh, Math.round(x - rsz / 2), Math.round(y - rsz / 2), rsz, rsz);
  } else {
...
// Returns true if sprite was used
// Draws 1px overlap on each side to prevent sub-pixel seams
function drawTileSprite(x, y, tileSize, variant) {
  const sprId = variant ? 'tile_grass2' : 'tile_grass';
  if (!SPR[sprId]) return false;
  X.imageSmoothingEnabled = false;
  X.drawImage(SPR[sprId], x - 1, y - 1, tileSize + 2, tileSize + 2);
  return true;
}

// Extract first frame from spritesheet as data URL (cached for HTML icons)
const _frameCache = {};
function getFirstFrameURL(sprId) {
  if (_frameCache[sprId]) return _frameCache[sprId];
  const img = SPR[sprId];
  if (!img) return null;
  const sd = SHEET[sprId];
  const fw = sd ? sd.fw : img.width;
  const fh = sd ? sd.fh : img.height;
...
function getSkillIconHTML(skillId, emojiIcon, size) {
  ...
  if (sprId && SPR[sprId]) {
    const src = getFirstFrameURL(sprId);
    return `<img src="${src}" width="${size || 24}" height="${size || 24}" style="image-rendering:pixelated;vertical-align:middle">`;
  }
...
function getCharIconHTML(charId, emojiIcon, size) {
  ...
  if (sprId && SPR[sprId]) {
    const src = getFirstFrameURL(sprId);
    return `<img src="${src}" width="${size || 32}" height="${size || 32}" style="image-rendering:pixelated;vertical-align:middle">`;
  }
  return emojiIcon;
}
